...
def emb_init(name, feat_num, embedding_size=-1, initializer=None, zero_first_row=True, pre_trained=False, trained_emb_path=None):
    if not pre_trained:
        if not initializer:
        ...
        if embedding_size <= 0:
            ...
            mat_shape = [feat_num, embedding_size]
        ...
        if zero_first_row:  # The first row of initialization is zero
            if embedding_size <= 0:
                ...
                mat_shape = [1, embedding_size]
            embeddings = tf.concat((tf.zeros(shape=mat_shape), embeddings[1:]), 0)
    else:
...
def multi_cate_emb_dense(multi_feats_range, init_emb, multi_cate_feats):
    all_multi_emb = None
...
def multi_cate_emb_sparse(multi_feats_range, init_emb, multi_cate_feats):
    all_multi_emb = None
    start_pos = 0
    for item in multi_feats_range:
        index_start = int(item[0])
        ...
        multi_emb = tf.nn.embedding_lookup(init_emb, ids=multi_cate_feats[:, index_start:index_end])
        # multi_emb = tf.reduce_sum(multi_emb, axis=1)  # sum-pool
        multi_emb = tf.reduce_mean(multi_emb)  # mean-pool
