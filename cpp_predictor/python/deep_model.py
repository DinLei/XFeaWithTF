...
class Model:
    ...
    def concat(self, fields, sparse_ids, sparse_vals):
        ...
        for i, field_id in enumerate(fields):
            ...
            table = tf.contrib.lookup.index_table_from_tensor(mapping=mapping_ints, num_oov_buckets=100000,
                                                              dtype=tf.int64)
            sparse_id_in_this_field = table.lookup(sparse_ids[i])
            with tf.variable_scope("emb_" + str(field_id)):
                embedding_variable = tf.get_variable(name="emb_" + str(field_id),
                                                     initializer=tf.truncated_normal([100002, self.embedding_size],
                                                                                     stddev=0.1))
                embedding = tf.nn.embedding_lookup_sparse(embedding_variable, sparse_id_in_this_field, sparse_vals[i],
                                                          combiner="sum")
    ...
    def forward(self, sparse_id, sparse_val):
        ...
        with tf.variable_scope("forward"):
            ...
            for i, hidden_size in enumerate(self.hidden_layer):
                ...
                weight = tf.get_variable(initializer=tf.truncated_normal([dim, hidden_size], stddev=0.1),
                                         name='fully_weight_' + str(i))
                bias = tf.get_variable(initializer=tf.truncated_normal([hidden_size], stddev=0.1),
                                       name='fully_bias_' + str(i))
